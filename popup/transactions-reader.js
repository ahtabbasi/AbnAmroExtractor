class Transaction {
  ...
  print() {
    console.log(
      `date ${this.date} | item ${this.item} | amount ${this.amount} | comments ${this.comments}`
    );
  }
...
function parseData() {
  let transactionTiles = document.querySelectorAll("button.transaction-tile");
  let allTransactions = [];
  ...
  transactionTiles.forEach((tile) => {
    try {
      ...
      // Safely get amount
      let amountElement = tile.querySelector(
